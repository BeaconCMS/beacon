defmodule Beacon.SEO.JsonLdTest do
  ...
  describe "article_schema/3" do
    test "builds Article schema from page manifest" do
      manifest = %{
        title: "My Article",
        og_title: nil,
        ...
        updated_at: ~N[2026-04-10 14:00:00]
      }

      schema = JsonLd.article_schema(manifest, config(%{site_name: "Test Site"}), @base_url)

      assert schema["@context"] == "https://schema.org"
      assert schema["@type"] == "Article"
      assert schema["headline"] == "My Article"
      assert schema["description"] == "A great article"
      assert schema["image"] == "https://example.com/img.jpg"
      assert schema["url"] == "https://example.com/blog/my-article"
      assert schema["datePublished"] == "2026-01-15T10:30:00"
      assert schema["dateModified"] == "2026-04-10T14:00:00"
      assert schema["publisher"]["@type"] == "Organization"
      assert schema["publisher"]["name"] == "Test Site"
    end

    test "prefers og_title over title for headline" do
      manifest = %{
        title: "Page Title",
        og_title: "OG Title",
      ...
      schema = JsonLd.article_schema(manifest, config(), @base_url)
      assert schema["headline"] == "OG Title"
    end

    test "generates URL from base_url + path when no canonical_url" do
      manifest = %{
        title: "Test",
        og_title: nil,
        meta_description: nil,
      ...
      schema = JsonLd.article_schema(manifest, config(), @base_url)
      assert schema["url"] == "https://example.com/blog/test"
    end
  end
