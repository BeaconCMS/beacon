defmodule Beacon.Content do
  ...
  @doc """
  Creates a component attr.

  ## Example

      iex> create_component_attr(attrs)
      {:ok, %ComponentAttr{}}

  """
  @spec create_component_attr(map()) :: {:ok, ComponentAttr.t()} | {:error, Changeset.t()}
  @doc type: :component_attrs
  def create_component_attr(attrs \\ %{}) do
    %ComponentAttr{}
    |> ComponentAttr.changeset(attrs)
    |> Repo.insert()

    # Do I need to do this for component_attr?
    #  |> tap(&maybe_broadcast_updated_content_event(&1, :component))
  end
  ...
  """
  @doc type: :component_attrs
  @spec update_component_attr(ComponentAttr.t(), map()) :: {:ok, ComponentAttr.t()} | {:error, Changeset.t()}
  def update_component_attr(%ComponentAttr{} = component_attr, attrs) do
    component_attr
    |> ComponentAttr.changeset(attrs)
    |> Repo.update()
  ...
  @doc """
  Deletes a component attr.
  """
  @doc type: :component_attrs
  @spec delete_component_attr(ComponentAttr.t()) :: {:ok, ComponentAttr.t()} | {:error, Changeset.t()}
  def delete_component_attr(component_attr) do
    Repo.delete(component_attr)
  end

  @doc """
  List component attrs.
  ...
  """
  @doc type: :component_attrs
  @spec list_component_attrs(String.t(), keyword()) :: [ComponentAttr.t()]
  def list_component_attrs(component_id, opts \\ []) do
    per_page = Keyword.get(opts, :per_page, 20)
    page = Keyword.get(opts, :page, 1)
    search = Keyword.get(opts, :query)
    sort = Keyword.get(opts, :sort, :name)
    ...
    component_id
    |> query_list_component_attrs_base()
    |> query_list_component_attrs_limit(per_page)
    |> query_list_component_attrs_offset(per_page, page)
    |> query_list_component_attrs_search(search)
    |> query_list_component_attrs_sort(sort)
    |> Repo.all()
  end

  defp query_list_component_attrs_base(component_id), do: from(c in ComponentAttr, where: c.component_id == ^component_id)

  defp query_list_component_attrs_limit(query, limit) when is_integer(limit), do: from(q in query, limit: ^limit)
  defp query_list_component_attrs_limit(query, :infinity = _limit), do: query
  defp query_list_component_attrs_limit(query, _per_page), do: from(q in query, limit: 20)

  defp query_list_component_attrs_offset(query, per_page, page) when is_integer(per_page) and is_integer(page) do
    offset = page * per_page - per_page
    from(q in query, offset: ^offset)
  end

  defp query_list_component_attrs_offset(query, _per_page, _page), do: from(q in query, offset: 0)

  defp query_list_component_attrs_search(query, search) when is_binary(search), do: from(q in query, where: ilike(q.name, ^"%#{search}%"))
