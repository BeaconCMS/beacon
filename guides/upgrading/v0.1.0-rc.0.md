...
Some changes are required in your site configuration for this version:

1. Add a `:router` key pointing to your app router module.
2. Add `:repo` key pointing to your app repo module.
2. Remove the `:data_source` key.

Edit your site(s) configuration, usually in the file `application.ex` to add those fields if not present.
...
To make it easier to manage the configuration, we're now using the `config/runtime.exs` file to define it.
...
Now open the file `config/runtime.exs` and add copy the configuration you had in `application.ex` to this file,
and also add the new required `:repo` key that points to your app repo module.
